...
const CalendarEvents = ({ calendarId, title = "Upcoming Events", maxResults = 10 }) => {
  ...
  useEffect(() => {
    const fetchGCAL = async () => {
      try {
        if (!calendarId) throw new Error("No calendarId provided");
  ...
  return (
    <section className="w-full bg-gray-50 text-gray-900 px-6 py-20">
        ...
        <div className="space-y-6">
          {events.map((event, index) => (
            ...
            >
                ...
                {event.location &&
                  (isURL(event.location) ? (
                    <a
                      ...
                      className="text-medcsblue hover:underline"
                    >
                      {event.location}
                    </a>
                  ) : (
              ...
              {/* Render HTML safely from Google Calendar (trusted source) */}
              {event.description && (
                <p
